(() => {
  ...
  async function loadAdminSystem(){
    ...
    if($('testOpsChatSettings'))$('testOpsChatSettings').addEventListener('click',testAdminOpsChatSettings);
  }
  async function testAdminOpsChatSettings(){
    const btn=$('testOpsChatSettings'); if(btn) btn.disabled=true;
    try{
      const result=await api('/api/admin/ops-chat/test',{method:'POST'});
      const detail=result&&result.detail?String(result.detail):'';
      toast(detail||(result&&result.ok?tr('opsChatTestSent'):tr('opsChatTestFailed')),result&&result.ok?'success':'error');
    }catch(e){handleApiError(e);}
