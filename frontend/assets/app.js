(() => {
  ...
  function renderAdminUserModal(d){
    const u=d.user||{}, stats=d.stats||{}, ref=d.referrer, adjustments=d.balance_adjustments||[], referrerAdjustments=d.referrer_adjustments||[], referralAdjustments=d.referral_balance_adjustments||[], levelAdjustments=d.referral_level_adjustments||[], manualLevel=Number((d.referral_level_override||{}).unlocked_level||0); const name=u.first_name||u.username||u.telegram_id;
    ...
    $('adminUserDetail').innerHTML=`<div class="user-detail-head"><div class="profile-avatar">${esc(String(name)[0]||'U')}</div><div><strong>${esc(name)}</strong><span>${u.username?'@'+esc(u.username)+' · ':''}ID ${esc(u.telegram_id)}</span></div></div>
      ...
      <div class="modal-section user-control-section"><h4>${esc(tr('referralBalanceControl'))}</h4><div class="big-balance">${money(d.referral_balance_minor)} <small>USDT</small></div><p class="field-hint">${esc(tr('referralBalanceNote'))}</p><label class="input-label" for="adminReferralBalanceInput">${esc(tr('referralBalanceControl'))}, USDT</label><input id="adminReferralBalanceInput" class="text-input" type="number" min="0" step="0.000001" inputmode="decimal" value="${esc((Number(d.referral_balance_minor||0)/1000000).toFixed(6))}"><label class="input-label" for="adminReferralBalanceReason">${esc(tr('adminReason'))}</label><textarea id="adminReferralBalanceReason" class="textarea-input compact-textarea" maxlength="500" rows="2" placeholder="${esc(tr('adminReasonHint'))}"></textarea><button id="saveAdminReferralBalance" class="primary-btn compact-primary" type="button">${esc(tr('setBalance'))}</button><h4>${esc(tr('referralBalanceHistory'))}</h4><div class="balance-audit-list">${referralBalanceHistory}</div></div>
    ...
    $('saveAdminReferralBalance').addEventListener('click',()=>setAdminReferralBalance(u.telegram_id));
    $('openAdminInvestment').addEventListener('click',()=>openAdminInvestment(u.telegram_id));
    $('saveAdminReferralLevel').addEventListener('click',()=>setAdminReferralLevel(u.telegram_id));
  ...
  async function setAdminReferralBalance(id){
    ...
    if(!Number.isFinite(value)||value<0||!reason){toast(tr('invalidData'),'error');return;}
    try{await api(`/api/admin/users/${encodeURIComponent(id)}/referral-balance`,{method:'POST',body:JSON.stringify({balance_usdt:raw,reason})});toast(tr('referralBalanceSaved'),'success');await openAdminUser(id);}catch(e){handleApiError(e);}
  }
  ...
  async function setAdminReferralLevel(id){
    ...
    try{await api(`/api/admin/users/${encodeURIComponent(id)}/referral-level`,{method:'POST',body:JSON.stringify({unlocked_level,reason})});toast(tr('levelAccessSaved'),'success');await openAdminUser(id);}catch(e){handleApiError(e);}
  }
  ...
  async function loadAdminDeposits(){
    ...
    $('adminDepositsList').innerHTML=rows.length?rows.map((d)=>`<article class="list-card"><div class="list-card-header"><div><strong>${money(d.principal_minor)} USDT</strong><small>${esc(d.first_name||d.username||d.user_id)} · ID ${esc(d.user_id)} · #${esc(d.id)}</small></div><span class="tag ${esc(d.status)}">${esc(statusText(d.status))}</span></div><small>${esc(fmtDate(d.opened_at))}${d.source==='admin'?' · '+esc(tr('adminSource')):(d.tx_hash?' · '+esc(compactAddress(d.tx_hash)):'')}</small></article>`).join(''):`<div class="empty-state">${esc(tr('noDeposits'))}</div>`;
  }
